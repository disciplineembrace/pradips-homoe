#!/usr/bin/env python3
"""Merge Mathur v2 into master remedies.json."""
...
V2_FILE = '/home/z/my-project/data/mm-v2/mathur-v2.json'

with open(REMEDIES_JSON) as f:
    remedies = json.load(f)
print(f"Loaded {len(remedies):,} remedies")

backup = REMEDIES_JSON + f'.backup-pre-mathur-v2-{datetime.now().strftime("%Y%m%d-%H%M%S")}'
shutil.copy2(REMEDIES_JSON, backup)
print(f"Backup: {backup}")

before = len(remedies)
remedies = [r for r in remedies if r.get('author') != 'Mathur']
print(f"Removed {before - len(remedies)} old Mathur entries")

with open(V2_FILE) as f:
    new_data = json.load(f)
for r in new_data:
    remedies.append(r)
print(f"Added {len(new_data)} new Mathur entries")
...
print("\n=== Sample: Mathur Abrotanum ===")
for r in remedies:
    if r.get('author') == 'Mathur' and 'abrotanum' in r.get('name', '').lower():
        print(f"  name: {r['name']}")
