 ...
 * HIGH → RED, LOW → GREEN, LOWER → BLUE, NORMAL → BLACK
 */
export type NormalizedGradeLevel = 'HIGH' | 'LOW' | 'LOWER' | 'NORMAL' | 'NOT_SPECIFIED';
export interface GradeDisplayConfig { color: string; bg: string; border: string; label: string; cssClass: string; weight: string; }
