...
export async function GET(req: NextRequest) {
  ...
  if (author) rubrics = rubrics.filter(r => r.source === author || (r as any).author === author);
  if (q) {
    rubrics = rubrics.filter(r => {
      const remedyText = (r.remedies || []).map((rem: string) => rem.split('|')[0]).join(' ');
  ...
  const items = rubrics.slice(start, start + pageSize).map(r => ({
    ...
    chapter: r.chapter,
    level: r.level,
    remedies: (r.remedies || []).map((rem: string) => rem.split('|')[0]),
  }));
