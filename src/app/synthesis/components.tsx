...
export const Icons = {
  Rubrics: ({ size = 20, className = '' }: IconProps) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#124C3B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
  ...
  HowItWorks: ({ size = 20, className = '' }: IconProps) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#C49A3A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
    ...
    <div className="flex flex-col items-center justify-center py-8">
      <div className="relative w-16 h-16">
        ...
        <svg className="w-16 h-16 animate-spin" viewBox="0 0 64 64" style={{ animationDuration: '2s' }}>
          ...
          <circle
            cx="32" cy="32" r="28" fill="none" stroke="#124C3B" strokeWidth="3"
            strokeLinecap="round"
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
            <path d="M12 2C8 6 6 10 6 14c0 4 2 8 6 8s6-4 6-8c0-4-2-8-6-12z" fill="#C49A3A" opacity="0.8"/>
            <path d="M12 6v14" stroke="#124C3B" strokeWidth="1" opacity="0.4"/>
          </svg>
        </div>
      </div>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4">
      <div className="w-16 h-16 rounded-full bg-[#FAF8F2] border border-[#E5DCC8] flex items-center justify-center mb-4">
        <span className="text-3xl">{icon}</span>
      </div>
      <h3 className="font-serif text-lg text-[#124C3B] mb-2">{title}</h3>
      {message && <p className="text-sm text-[#6B7280] text-center max-w-sm mb-4">{message}</p>}
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="px-5 py-2 bg-[#124C3B] text-white rounded-lg text-sm font-semibold hover:bg-[#123C30] transition-colors"
        >
          {actionLabel}
        </button>
      )}
            ...
            <React.Fragment key={step.num}>
              <div className="flex flex-col items-center gap-1 flex-shrink-0">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    isCompleted
                      ...
                      : isActive
                        ? 'bg-[#124C3B] text-white ring-2 ring-[#C49A3A] ring-offset-2'
                ...
                <span className={`text-[0.6rem] font-medium whitespace-nowrap ${
                  isActive ? 'text-[#124C3B]' : isCompleted ? 'text-[#22C55E]' : 'text-stone-400'
                }`}>
                  {step.label}
                </span>
              </div>
