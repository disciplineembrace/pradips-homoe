  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-2 md:p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="px-5 py-3 bg-[#124C3B] text-white flex items-center justify-between">
          <div>
        ...
        <div className="h-1.5 bg-stone-200">
          <div
            className="h-full bg-[#C49A3A] transition-all duration-300"
              ...
              >
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    idx === currentStep
                      ? 'bg-[#124C3B] text-white scale-110'
                      : idx < currentStep
                ...
                <span className={`text-[0.6rem] hidden md:block ${idx === currentStep ? 'text-[#124C3B] font-semibold' : 'text-stone-400'}`}>
                  {s.title.split(' ')[0]}
                </span>
              </button>
            ))}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="flex items-start gap-4 mb-4">
            <div className="w-14 h-14 rounded-full bg-[#124C3B] flex items-center justify-center text-2xl flex-shrink-0">
              {step.icon}
            </div>
            <div className="flex-1">
              <div className="text-xs text-[#C49A3A] font-semibold uppercase tracking-wider mb-1">
                Step {step.num}
              </div>
              <h3 className="font-serif text-xl text-[#124C3B] mb-1">{step.title}</h3>
          ...
          <div className="space-y-2 mt-4">
            {step.details.map((detail, idx) => (
              <div key={idx} className="flex items-start gap-2.5 p-2.5 bg-stone-50 rounded-lg border border-stone-200">
                <span className="w-5 h-5 rounded-full bg-[#C49A3A] text-[#124C3B] flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                  {idx + 1}
                </span>
          ...
          <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: '#EAF4EF', border: '1px solid rgba(15, 74, 56, 0.2)' }}>
            <div className="text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#124C3B' }}>💡 Tip</div>
          ...
          <div className="flex gap-2">
            ...
            {isLast ? (
              <button
                ...
                className="px-5 py-2 text-sm font-semibold bg-[#124C3B] text-white rounded-lg hover:bg-[#0B392D] transition-colors"
              >
                ✓ Start Repertorizing →
              </button>
            ) : (
