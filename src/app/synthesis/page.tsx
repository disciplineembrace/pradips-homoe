...
type SavedCase = { name: string; notes: string; date: string; rubrics: SelectedRubric[]; results: RepertResult[]; patient?: any; caseNo?: string };
      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 py-4 w-full pb-20">
        ...
        {screen === 'casePaper' && (<div className="space-y-3">
          ...
          <div className="bg-white rounded-lg shadow p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div><label className="text-xs text-[#7C8F6E]">Case No</label><input type="text" value={patient.caseNo} onChange={e => setPatient({...patient, caseNo: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
              <div><label className="text-xs text-[#7C8F6E]">Age</label><input type="text" value={patient.age} onChange={e => setPatient({...patient, age: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            </div>
        ...
        {screen === 'rubricDetail' && rubricDetail && (<div className="space-y-3">
          <div className="flex items-center gap-2"><button onClick={() => go(screen === 'rubricDetail' ? 'chapters' : 'search', 'chapters')} className="text-[#173B2D]">←</button><h2 className="font-serif text-lg text-[#173B2D] flex-1">{rubricDetail.rubric?.name}</h2></div>
        ...
        {screen === 'report' && repertResults.length > 0 && (<div className="space-y-3">
          <div className="flex items-center gap-2"><button onClick={() => go('results', 'case')} className="text-[#173B2D]">←</button><h2 className="font-serif text-lg text-[#173B2D] flex-1">Repertorization Report</h2></div>
          <div className="bg-white rounded-lg shadow p-6 print:p-8" id="report-content">
            {/* Header */}
            <div className="text-center mb-4 print:mb-6">
              {profile.clinicName && <h2 className="font-serif text-xl text-[#173B2D]">{profile.clinicName}</h2>}
              {profile.doctorName && <p className="text-sm text-[#173B2D] font-semibold">{profile.doctorName}</p>}
              {profile.qualification && <p className="text-xs text-[#7C8F6E]">{profile.qualification}</p>}
              {profile.address && <p className="text-xs text-[#7C8F6E]">{profile.address}</p>}
              {profile.phone && <p className="text-xs text-[#7C8F6E]">Phone: {profile.phone}</p>}
              <div className="border-t border-[#E8DCC3] mt-3 pt-3">
                <h3 className="font-serif text-lg text-[#173B2D]">Repertorization Sheet</h3>
              </div>
            </div>
            {/* Patient Details */}
            <div className="mb-4 print:mb-6">
              <h4 className="text-xs font-bold text-[#7C8F6E] uppercase mb-2">Patient Details</h4>
              <div className="grid grid-cols-2 gap-2 text-sm">
                {patient.name && <div><span className="text-[#7C8F6E]">Name:</span> <span className="text-[#173B2D] font-medium">{patient.name}</span></div>}
                {patient.caseNo && <div><span className="text-[#7C8F6E]">Case No:</span> <span className="text-[#173B2D]">{patient.caseNo}</span></div>}
                {patient.age && <div><span className="text-[#7C8F6E]">Age:</span> <span className="text-[#173B2D]">{patient.age}</span></div>}
                {patient.sex && <div><span className="text-[#7C8F6E]">Sex:</span> <span className="text-[#173B2D]">{patient.sex}</span></div>}
                <div><span className="text-[#7C8F6E]">Date:</span> <span className="text-[#173B2D]">{patient.date}</span></div>
              </div>
            </div>
            {/* Selected Rubrics */}
            <div className="mb-4 print:mb-6">
              <h4 className="text-xs font-bold text-[#7C8F6E] uppercase mb-2">Selected Rubrics</h4>
              <div className="space-y-1">{selectedRubrics.filter(r => r.enabled).map((r, i) => (<div key={r.id} className="text-sm flex justify-between"><span className="text-[#173B2D]">{i + 1}. {r.path}</span><span className="text-[#7C8F6E]">W: {r.weight}</span></div>))}</div>
            </div>
            {/* Results Table */}
            <div className="mb-4 print:mb-6">
              <h4 className="text-xs font-bold text-[#7C8F6E] uppercase mb-2">Repertorization Analysis</h4>
              <table className="w-full text-xs print:text-sm"><thead><tr className="border-b border-[#E8DCC3]"><th className="text-left py-1">Rank</th><th className="text-left py-1">Remedy</th><th className="text-left py-1">Score</th><th className="text-left py-1">Coverage</th></tr></thead><tbody>{repertResults.slice(0, 20).map((r, i) => (<tr key={r.abbrev} className="border-b border-[#E8DCC3]"><td className="py-1 text-[#7C8F6E]">{i + 1}</td><td className="py-1 text-[#173B2D] font-medium">{r.abbrev}</td><td className="py-1 text-[#173B2D]">{r.totalScore}</td><td className="py-1 text-[#173B2D]">{r.coverage}</td></tr>))}</tbody></table>
            </div>
            {/* Grade Legend */}
          ...
          <div className="flex gap-2"><button onClick={() => window.print()} className="flex-1 bg-[#173B2D] text-[#C8A24A] py-2.5 rounded-lg text-sm">🖨 Print</button><button onClick={() => go('saveHistory', 'more')} className="flex-1 bg-white border border-[#E8DCC3] text-[#173B2D] py-2.5 rounded-lg text-sm">Save Case</button></div>
        </div>)}
        ...
        {/* PROFILE */}
        {screen === 'profile' && (<div className="space-y-3">
          <div className="flex items-center gap-2"><button onClick={() => go('dashboard', 'home')} className="text-[#173B2D]">←</button><h2 className="font-serif text-lg text-[#173B2D] flex-1">Report Profile</h2></div>
          <div className="bg-white rounded-lg shadow p-4 space-y-3">
            <p className="text-xs text-[#7C8F6E]">Configure your professional identity for repertorization reports.</p>
            <div><label className="text-xs text-[#7C8F6E]">Doctor Name</label><input type="text" value={profile.doctorName || ''} onChange={e => setProfile({...profile, doctorName: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            <div><label className="text-xs text-[#7C8F6E]">Qualification</label><input type="text" value={profile.qualification || ''} onChange={e => setProfile({...profile, qualification: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            <div><label className="text-xs text-[#7C8F6E]">Clinic Name (optional)</label><input type="text" value={profile.clinicName || ''} onChange={e => setProfile({...profile, clinicName: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            <div><label className="text-xs text-[#7C8F6E]">Clinic Address (optional)</label><textarea value={profile.address || ''} onChange={e => setProfile({...profile, address: e.target.value})} rows={2} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            <div><label className="text-xs text-[#7C8F6E]">Phone (optional)</label><input type="text" value={profile.phone || ''} onChange={e => setProfile({...profile, phone: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            <div><label className="text-xs text-[#7C8F6E]">Email (optional)</label><input type="text" value={profile.email || ''} onChange={e => setProfile({...profile, email: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
            <div><label className="text-xs text-[#7C8F6E]">Registration No (optional)</label><input type="text" value={profile.regNo || ''} onChange={e => setProfile({...profile, regNo: e.target.value})} className="w-full px-3 py-2 border border-[#E8DCC3] rounded text-sm text-[#173B2D]" /></div>
          </div>
          <button onClick={() => go('dashboard', 'home')} className="w-full bg-[#173B2D] text-[#C8A24A] py-2.5 rounded-lg text-sm font-semibold">Save Profile</button>
        </div>)}
