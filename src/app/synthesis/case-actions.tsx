      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-stone-200 bg-stone-50 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2">
            <Icons.Pencil size={18} className="text-[#173B2D]" />
            <h2 className="font-serif text-lg text-[#173B2D]">Edit Saved Case</h2>
          </div>
          <button
            onClick={onCancel}
            disabled={saving}
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              <span className="inline-block w-6 h-6 border-2 border-[#173B2D]/30 border-t-[#173B2D] rounded-full animate-spin mb-3" />
              ...
              <fieldset className="space-y-3" disabled={saving}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Field label="Patient Name" required={false}>
                    <input
                      ...
                      className="w-full px-3 py-2 border border-stone-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#173B2D]/30 focus:border-[#173B2D]"
                    />
                  </Field>
                  ...
                  <Field label="Sex">
                    <select
                      ...
                      className="w-full px-3 py-2 border border-stone-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#173B2D]/30 focus:border-[#173B2D]"
                    >
                      {SEX_OPTIONS.map(s => (
                ...
                <Field label="Notes">
                  <textarea
                    ...
                    className="w-full px-3 py-2 border border-stone-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#173B2D]/30 focus:border-[#173B2D] resize-y"
                  />
                </Field>
              </fieldset>
                    ...
                    {rubrics.map(r => (
                      <div key={r.symptomId} className="flex items-center gap-2 px-3 py-2">
                        <input
                          ...
                          className="w-4 h-4 accent-[#173B2D] flex-shrink-0"
                      ...
                      <tbody className="divide-y divide-stone-100">
                        {results.slice(0, 10).map((r, i) => (
                          <tr key={r.abbrev}>
                            ...
                            <td className="px-2 py-1 text-right font-medium text-[#173B2D]">{r.totalScore}</td>
        ...
        <div className="px-4 py-3 border-t border-stone-200 bg-stone-50 flex items-center justify-end gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-[#173B2D] rounded-md hover:bg-[#0f2a20] disabled:opacity-50 disabled:cursor-not-allowed min-w-[120px] justify-center"
          >
            {saving ? (
              <>
    ...
    >
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-stone-200 flex items-center gap-3">
          ...
          <h2 className="font-serif text-lg text-[#173B2D]">Delete Saved Case?</h2>
        </div>
