    ...
    <div className="min-h-screen flex flex-col bg-[#F5EFE0]">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 py-8 w-full">
        <div className="flex items-center justify-between mb-6">
          <h1 className="font-serif text-3xl text-[#173B2D]">Create User</h1>
        ...
        {success && (
          <div className="bg-emerald-50 border-2 border-emerald-400 rounded-lg p-6 mb-6">
            <h2 className="font-serif text-xl text-[#173B2D] mb-2">✓ User Created</h2>
            ...
            <div className="bg-white rounded p-4 mb-4 grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-[#7C8F6E]">Name:</span> <b>{success.user.name}</b></div>
              <div><span className="text-[#7C8F6E]">Role:</span> <b>{success.user.role}</b></div>
              {success.user.email && <div><span className="text-[#7C8F6E]">Email:</span> {success.user.email}</div>}
              <div><span className="text-[#7C8F6E]">Status:</span> <b>{success.user.status}</b></div>
            </div>
            <div className="bg-amber-100 border-2 border-amber-400 rounded p-4 text-center">
              <div className="text-xs text-[#5a6b50] uppercase tracking-wider mb-1">6-digit PIN</div>
              <div className="font-mono text-4xl tracking-widest text-[#C8A24A] font-bold">{success.generatedPin}</div>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-3 py-2 border-2 border-[#E8DCC3] rounded focus:outline-none focus:border-[#173B2D]"
              ...
              <div>
                ...
                />
                <p className="text-xs text-[#7C8F6E] mt-1">User logs in with Email + PIN.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full px-3 py-2 border-2 border-[#E8DCC3] rounded focus:outline-none focus:border-[#173B2D]">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-semibold text-[#5a6b50] uppercase">6-digit PIN *</label>
                  <button
                    type="button"
                    onClick={() => { const p = generateRandomPin(); setForm({ ...form, pin: p }); setShowPin(true); }}
                    className="text-xs bg-[#C8A24A] hover:bg-[#d4b560] text-white px-3 py-1 rounded font-semibold"
                  >🎲 Generate PIN</button>
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    inputMode="numeric"
                    maxLength={6}
                    className="flex-1 px-3 py-2 border-2 border-[#E8DCC3] rounded focus:outline-none focus:border-[#173B2D] font-mono text-lg tracking-widest text-center"
                    placeholder="------"
                  />
                ...
                <p className="text-xs text-[#7C8F6E] mt-1">Must be unique. Click Generate for a random PIN. PIN is shown only once after creation.</p>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="bg-[#173B2D] hover:bg-[#2a5443] disabled:opacity-50 text-white px-6 py-2 rounded font-semibold"
                >
