...
export default function AdminUsersPage() {
  ...
  async function toggleBlock(u: User) {
    if (!confirm(`${u.status === 'active' ? 'Block' : 'Unblock'} user ${u.loginId}?`)) return;
    ...
    <div className="min-h-screen bg-stone-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-serif text-2xl text-emerald-900">Users ({users.length})</h2>
          <Link href="/admin/users/create" className="bg-emerald-900 hover:bg-emerald-800 text-white text-sm px-4 py-2 rounded">+ Create User</Link>
        </div>
                ...
                {users.map(u => {
                  ...
                  return (
                    <tr key={u.id} className="border-b border-stone-200 hover:bg-stone-50">
                      ...
                      <td className="p-3">
                        <span className={`text-xs px-2 py-1 rounded ${u.role === 'admin' ? 'bg-amber-100 text-amber-800' : 'bg-stone-200 text-stone-700'}`}>{u.role}</span>
                      </td>
                      <td className="p-3">
                        <span className={`text-xs ${u.status === 'active' ? 'text-emerald-700' : 'text-red-700'}`}>{u.status}</span>
                      </td>
                      <td className="p-3">
                        {isLocked ? (
                      ...
                      <td className="p-3">
                        <div className="flex flex-wrap gap-1">
                          ...
                          <button onClick={() => toggleBlock(u)} className={`text-xs px-2 py-1 rounded text-white ${u.status === 'active' ? 'bg-red-700 hover:bg-red-600' : 'bg-emerald-700 hover:bg-emerald-600'}`}>
